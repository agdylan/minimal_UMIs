### This file allows you to run our estimator on minimal dataset 
...
adata = sc.read_h5ad(f"{base_dir}/{dataset}/ub_objects/adata_matrices/adata_5.h5ad")



### Code for generating UMI probs for length 5 
def generate_umis_and_probs():
    ...
    log_probs = [sum(nt_logs[b] for b in umi) for umi in umis]
    probs = np.exp(log_probs)

    return pd.DataFrame({'UMI': umis, 'prob': probs})
...
def predict_with_collision_estimator(chosen_k, adata):
    """
    Runs the non-uniform UMI collision-aware estimator end-to-end
    for a given k and input AnnData object.
    Returns a new AnnData with predicted counts.
    """

    ## Generate UMI probabilities for the chosen k
    umi_probs = generate_umis_and_probs()  # returns df for k only
    umi_probs = umi_probs['prob']
    ## Extract matrix
    ...
    print("Successfully predicted counts with collision-aware estimator.")
    return predicted_adata

predict_with_collision_estimator(chosen_k=5, adata=adata)